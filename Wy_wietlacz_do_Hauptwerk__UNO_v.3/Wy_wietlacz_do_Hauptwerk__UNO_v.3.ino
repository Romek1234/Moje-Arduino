...
const byte LED=13; // dioda led
const byte MIDI_SYSEX=0xF0; //code for SysEx command
...
void loop() { 
  ...
  lcd.print(midiType);
    ...
    if (midiType == MIDI_SYSEX) { 
      digitalWrite(LED, HIGH);   // ============turn the LED on (HIGH is the voltage level)
       
    //this is a SysEx message and will fit in our array   
    unsigned int wiel_tabl = MIDI.getSysExArrayLength();
    const byte *sysexmessage = MIDI.getSysExArray();
    hwline1="";
    hwline2="";
       lcd.clear();
  lcd.setCursor(0, 0); //Ustawienie kursora
  lcd.print(sysexmessage[0],sysexmessage[1]);
  lcd.setCursor(0, 1); //Ustawienie kursora
  lcd.print(data1);
    if (1==1 ){// sysexmessage[1]==ID_HAUPTWERK) {
